...
configuration = {
    "name": "CopernicusNG SmartHouse living Room",
...
def main():
    ...
    def button1_pressed():
        cmd = "f1;livingRoom;lamp;1;switch";
        sockSnd.sendto(cmd.encode('utf-8'), (MCAST_GRP, MCAST_PORT))

    def button2_pressed():
        cmd = "f1;kitchen;lamp;1;switch";
        sockSnd.sendto(cmd.encode('utf-8'), (MCAST_GRP, MCAST_PORT))

    def handleCommand(cmd):
        if cmd[0] in {'f1', '*'} & cmd[1] in {'livingRoom', '*'} & cmd[2] in {"lamp", '*'} & cmd[3] in {'1', '*'}:
            if cmd[4] == "off":
                led1.off()
            elif cmd[4] == "on":
                led1.on()
